...
function MegaLink({ item }: { item: NavChild }) {
  return (
    <a
      href={item.href}
      className="tl-mega-link flex min-h-11 cursor-pointer items-start gap-3 rounded-xl px-3 py-2.5"
    >
      {item.icon ? (
        <span className="tl-mega-icon mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-[10px]">
          <IconsaxOutline name={item.icon} className="size-5" />
        </span>
      ) : null}
      <span className="min-w-0">
        <span className="tl-mega-label block text-[14px] leading-5 tracking-[0.16px]">{item.label}</span>
        {item.description ? (
          <span className="tl-mega-desc mt-0.5 block text-[12px] leading-[1.4] tracking-[0.02em]">
            {item.description}
          </span>
        ) : null}
      </span>
                  ...
                  <div key={item.label} className="group relative shrink-0">
                    ...
                    {mega ? (
                      <div className="invisible absolute top-full left-0 z-20 pt-3 opacity-0 transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
                        <div
                          className={`tl-mega rounded-2xl p-2 ${
